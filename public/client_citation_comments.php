...
<section class="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <div class="flex flex-wrap items-end justify-between gap-3">
        <div>
            <h1 class="text-xl font-extrabold text-slate-900">Client Citation Comments</h1>
        ...
        <form method="get" class="flex flex-wrap items-center gap-2">
            <input
                ...
                class="w-[21rem] max-w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-200"
            >
            ...
            <?php if ($search !== ''): ?>
                <a class="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100" href="<?php echo e(app_config()['base_url']); ?>/client_citation_comments.php">Reset</a>
            <?php endif; ?>
        </form>
    </div>
</section>
...
<section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <div class="mb-3 flex items-center justify-between gap-2">
        <p class="text-sm font-semibold text-slate-700">Total visible comments: <?php echo e((string)count($allCommentRows)); ?> · Tasks with comments: <?php echo e((string)count($groupedComments)); ?></p>
    ...
    <div class="cf-table-wrap overflow-x-auto rounded-xl border border-slate-200">
        <table class="cf-table w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Date</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500 hidden sm:table-cell">Client</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500 hidden md:table-cell">Business</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500 hidden lg:table-cell">Directory</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Citation</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500 hidden md:table-cell">Assigned Employee</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Comments</th>
                    <th class="px-4 py-3 text-right text-xs font-bold uppercase tracking-wide text-slate-500">Status</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 bg-white">
                <?php foreach ($groupedComments as $taskId => $comments): ?>
                    <?php
                        ...
                        $rowToneClass = $hasPending
                            ? 'odd:bg-amber-50/35 even:bg-amber-50/55'
                            : 'odd:bg-emerald-50/45 even:bg-emerald-50/70';
                    ?>
                    <tr class="<?php echo e($rowToneClass); ?>">
                        <td class="px-4 py-3 text-xs text-slate-600 whitespace-nowrap"><?php echo e((string)$summary['first_comment_at']); ?></td>
                        <td class="px-4 py-3 text-sm font-semibold text-slate-800 hidden sm:table-cell"><?php echo e((string)$summary['client_name']); ?></td>
                        <td class="px-4 py-3 text-sm text-slate-700 hidden md:table-cell">
                            <div class="flex items-center gap-2">
                                <?php if ($bizLogo !== ''): ?>
                                    <img class="h-7 w-7 shrink-0 rounded-md border border-slate-200 object-cover" src="<?php echo e(public_asset_url($bizLogo)); ?>" alt="<?php echo e((string)$summary['business_name']); ?> logo">
                                <?php else: ?>
                                    <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-slate-100 text-[11px] font-bold uppercase text-slate-600">
                        ...
                        <td class="px-4 py-3 text-sm text-slate-700 hidden lg:table-cell">
                            <div class="flex items-center gap-2">
                                <?php if ($dirLogo !== ''): ?>
                                    <img class="h-7 w-7 shrink-0 rounded-md border border-slate-200 object-cover" src="<?php echo e(public_asset_url($dirLogo)); ?>" alt="<?php echo e((string)$summary['directory_name']); ?> logo">
                                <?php else: ?>
                                    <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-slate-100 text-[11px] font-bold uppercase text-slate-600">
                        ...
                        <td class="px-4 py-3 text-sm text-slate-700">
                            <div class="inline-block">
                                <p class="font-semibold text-slate-800">#<?php echo e((string)(int)$taskId); ?></p>
                                <p class="text-xs text-slate-500"><?php echo e(status_label((string)($summary['citation_status'] ?? ''))); ?></p>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-sm text-slate-700 hidden md:table-cell"><?php echo e((string)($summary['assigned_to_name'] ?: 'Unassigned')); ?></td>
                        <td class="px-4 py-3 text-sm text-slate-700">
                            <button type="button" class="inline-flex items-center gap-1.5 rounded-lg border border-sky-300 bg-sky-50 px-2.5 py-1.5 text-xs font-semibold text-sky-700 hover:bg-sky-100 transition comment-modal-trigger" data-task-id="<?php echo e((string)$taskId); ?>">
                            ...
                            <div class="flex flex-col items-end gap-1">
                                <div class="flex items-center gap-1.5">
                                    <?php if ($taskPendingCount > 0): ?>
                                        <span class="inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-700">Pending: <?php echo e((string)$taskPendingCount); ?></span>
                                    <?php endif; ?>
                                    <?php if ($taskUpdatedCount > 0): ?>
                                        <span class="inline-flex items-center gap-0.5 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold text-emerald-700">
                ...
                <?php if (!$groupedComments): ?>
                    <tr>
                        <td class="px-4 py-4 text-sm text-slate-500" colspan="8">No client comments found for the current filters.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<div id="commentsModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 opacity-0 pointer-events-none transition-opacity duration-200" aria-hidden="true">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[85vh] overflow-y-auto">
        <div class="sticky top-0 bg-white border-b border-slate-200 px-5 py-4 flex items-center justify-between gap-3">
            <div>
                <h2 class="text-lg font-bold text-slate-900">Task #<span id="modalTaskId"></span> Comments</h2>
                <p class="mt-1 text-xs text-slate-500">All comments submitted for this citation</p>
            </div>
            <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition" id="closeCommentsModal" aria-label="Close modal">
...
(() => {
    ...
    const openModal = (taskId) => {
        ...
        comments.forEach((comment) => {
            ...
            if (isUpdated) {
                statusHtml = `<span class="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-700">
                ...
                if (comment.resolved_by_name) {
                    statusHtml += `<p class="mt-1 text-xs text-emerald-700">by ${comment.resolved_by_name}${comment.resolved_at ? ' · ' + comment.resolved_at : ''}</p>`;
                }
            } else {
                statusHtml = `<span class="inline-flex rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-700">Pending</span>`;
                ...
                actionHtml = `
                    <form method="post" class="mt-3">
                        ...
                        <button type="submit" class="inline-flex items-center gap-1.5 rounded-lg border border-sky-300 bg-sky-50 px-2.5 py-1.5 text-xs font-semibold text-sky-700 hover:bg-sky-100 transition">
            ...
            commentBox.innerHTML = `
                <div class="flex items-start justify-between gap-3 mb-2">
                    <p class="text-xs font-semibold ${isUpdated ? 'text-emerald-900' : 'text-amber-900'}">Comment on ${comment.created_at}</p>
                    ${statusHtml}
                </div>
                <p class="text-sm ${isUpdated ? 'text-emerald-900' : 'text-amber-900'} whitespace-pre-wrap break-words">${comment.comment_text}</p>
                ${actionHtml}
            `;
        ...
        if (comments.length === 0) {
            modalContent.innerHTML = '<p class="text-sm text-slate-500 py-4">No comments found for this task.</p>';
        }
