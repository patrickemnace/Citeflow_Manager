...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    set_flash('err', 'Invalid credentials.');
    redirect('/');
}
...
<head>
    ...
    <title>Sign In - <?php echo e((string)$config['app_name']); ?></title>
    ...
    <style>
        ...
        .cf-side-light::before{content:'';position:absolute;top:-10rem;left:-9rem;width:24rem;height:24rem;border-radius:999px;background:radial-gradient(circle,rgba(59,130,246,.12),rgba(59,130,246,0));pointer-events:none;}
        .cf-side-light::after{content:'';position:absolute;right:-8rem;bottom:-8rem;width:20rem;height:20rem;border-radius:999px;background:radial-gradient(circle,rgba(14,165,233,.1),rgba(14,165,233,0));pointer-events:none;}
        ...
        .cf-directory-gallery-empty{display:flex;height:clamp(14rem,34vh,22rem);align-items:center;justify-content:center;border:1px dashed rgba(148,163,184,.5);border-radius:1rem;color:#475569;font-size:.86rem;font-weight:600;background:rgba(255,255,255,.72);}
        .cf-column-stage{position:relative;z-index:1;width:min(100%,24rem);perspective:1400px;}
        .cf-column-card{position:relative;display:grid;transform-style:preserve-3d;transition:transform .62s cubic-bezier(.22,.61,.36,1);}
        .cf-side-dark.is-client .cf-column-card{transform:rotateY(180deg);}
        .cf-column-face{grid-area:1 / 1;backface-visibility:hidden;-webkit-backface-visibility:hidden;}
        .cf-column-face-back{transform:rotateY(180deg);}
        .cf-side-dark.is-client .cf-column-face-front{pointer-events:none;}
        .cf-side-dark:not(.is-client) .cf-column-face-back{pointer-events:none;}
        .cf-form-shell{position:relative;width:100%;}
        ...
        .dark .cf-side-light::before{background:radial-gradient(circle,rgba(56,189,248,.18),rgba(56,189,248,0));}
        .dark .cf-side-light::after{background:radial-gradient(circle,rgba(14,165,233,.15),rgba(14,165,233,0));}
        ...
        .dark .cf-directory-gallery-empty{border-color:rgba(71,85,105,.75);color:#cbd5e1;background:rgba(15,23,42,.72);}
    ...
    <main class="cf-split-shell">
        <section class="cf-side-light flex items-center px-6 py-4 sm:px-10 sm:py-6 lg:px-16 lg:py-8">
            <div class="relative z-10 mx-auto flex w-full max-w-2xl flex-col justify-between gap-6">
                <div class="max-w-lg">
                    <p class="text-sm font-semibold uppercase tracking-[0.28em] text-sky-600 dark:text-sky-300 cf-left-copy" data-left-portal>Staff Portal</p>
                    ...
                    <p class="mt-4 text-lg leading-8 text-slate-600 dark:text-slate-300 cf-left-copy" data-left-description>Manage citations, tasks, reports, and team activity from one focused workspace.</p>
                </div>
                <div class="cf-illustration-card p-4 sm:p-5">
                    ...
                    <?php else: ?>
                        <div class="cf-directory-gallery-empty">No active directory logos yet.</div>
                    <?php endif; ?>
                </div>
            </div>
        </section>
        <section class="cf-side-dark flex items-center justify-center px-6 py-4 sm:px-10 sm:py-6 lg:px-16 lg:py-8" data-login-column>
            <div class="cf-column-stage" data-login-flip data-login-default="staff">
                <div class="cf-column-card" data-login-card>
                    <article class="cf-form-shell text-white cf-column-face cf-column-face-front">
                        <div class="mb-6 flex items-start justify-between gap-4">
                            <div>
                                ...
                                <div class="mt-4 cf-login-tabs">
                                    <a class="cf-login-tab cf-login-tab-active" href="<?php echo e($base); ?>/" data-login-toggle="staff">Staff login</a>
                                    <a class="cf-login-tab" href="<?php echo e($base); ?>/client_portal.php" data-login-toggle="client">Client portal</a>
                                </div>
                            </div>
                            <button type="button" data-theme-toggle aria-label="Toggle dark and light mode" class="inline-flex items-center rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-white/15 dark:border-white/30 dark:bg-white/10 dark:text-white dark:hover:bg-white/20">
                    ...
                    <article class="cf-form-shell text-white cf-column-face cf-column-face-back">
                        <div class="mb-6 flex items-start justify-between gap-4">
                            <div>
                                <img src="<?php echo e($logoUrl); ?>" alt="CiteFlow Manager logo" class="h-12 w-auto max-w-[220px] brightness-0 invert">
                                <p class="mt-3 text-lg font-semibold tracking-tight text-white">CiteFlow Manager</p>
                        ...
                        <form method="post" action="<?php echo e($base); ?>/client_portal.php" class="space-y-3.5" data-inline-validation>
                            <div data-field-validation>
                                <label class="mb-2 block text-sm font-semibold text-white/88">Email Address</label>
                                <div class="relative">
    ...
    <script>
        (() => {
            ...
            const updateLeftCopy = (view) => {
                const copy = leftCopyByView[view] || leftCopyByView.staff;
                leftCopyNodes.forEach((node) => node.classList.add('is-updating'));

                window.setTimeout(() => {
                    if (leftPortal) {
                        leftPortal.textContent = copy.portal;
                        leftPortal.classList.toggle('text-sky-600', view !== 'client');
                        leftPortal.classList.toggle('dark:text-sky-300', view !== 'client');
                        leftPortal.classList.toggle('text-emerald-700', view === 'client');
                        leftPortal.classList.toggle('dark:text-emerald-300', view === 'client');
                    }
                    if (leftHeading) {
            ...
            const syncLabel = () => {
                const nextModeLabel = root.classList.contains('dark') ? 'Light' : 'Dark';
                themeToggleLabels.forEach((node) => {
